// src/pages/Programs.jsx — Sanity connected, no placeholders, no impact stories
...
const PROGRAMMES_QUERY = `
  *[_type == "programme"] | order(pillar asc) {
    _id, name, pillar, headline, headlineItalic,
    subheadline, whatWeDo, whoWeServe, donations,
    coverImage { asset ->{ _id, url }, alt }, impactStats
  }
`;
...
const PROGRAMME_STRUCTURE = [
  ...
  },
  {
    ...
    whatWeDo:     "The JOAM scholarship programme provides full and partial financial support to academically gifted students from low-income households. Coverage includes school fees, textbooks, examination registration, and mentorship. We follow our scholars — we do not fund one term and disappear.",
...
const METHODOLOGY = [
  { num: "01", headline: "Community-Rooted",        body: "Every programme we run is built in partnership with the communities we serve — not designed for them from the outside. We listen before we act. We stay after we start." },
  { num: "02", headline: "Long-Term by Design",      body: "We do not measure success by the number of one-time interventions we deliver. We measure it by what changes — and whether that change lasts." },
  { num: "03", headline: "Honest About Outcomes",    body: "We document what we do and report what we find — including when something does not work the way we intended. Transparency is not a marketing position for JOAM." },
  { num: "04", headline: "Built to Scale Carefully", body: "We are a new foundation. Every community we enter, we enter properly — with the resources, the people, and the commitment to do the work right." },
];
...
export default function Programs() {
  const { data: sanityProgrammes, loading } = useSanity(PROGRAMMES_QUERY);
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      {PROGRAMME_STRUCTURE.map((prog, i) => {
        const sanityData = sanityProgrammes?.find(p => p.name === prog.sanityName);
        return (
          <ProgrammeSection
            key={prog.id}
            prog={prog}
...
function ProgramsHero() {
  return (
    <section aria-label="Programs hero"
...
/* ── Programme Section ─────────────────────────────────────── */
function ProgrammeSection({ prog, sanityData, loading, bg }) {
  ...
  const donations   = sanityData?.donations?.length ? sanityData.donations : prog.donations;
      ...
      <div className="max-w-container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-10 items-start mb-14 md:mb-10">
          ...
          <motion.div
            ...
            className={prog.reverse ? "order-last md:order-first" : "order-first"}>
            <div className="w-full aspect-[4/3] bg-primary-light border border-primary/[0.08]
              rounded-[2px] relative overflow-hidden">
              {sanityData?.coverImage?.asset?.url ? (
                <img
                  src={urlFor(sanityData.coverImage).width(800).auto("format").url()}
              ...
              <div className="absolute bottom-4 right-5 opacity-[0.04]">
                <span className="font-display font-bold text-[64px] text-primary leading-none select-none">
                  {prog.id === "maternal" ? "01" : prog.id === "scholarships" ? "02" : "03"}
                </span>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className={prog.reverse ? "order-first md:order-last" : "order-last"}>
            <Overline color="gold">{prog.pillar} — {prog.officialName}</Overline>

            <h2 id={`${prog.id}-heading`}
            ...
            <div className="mb-5">
              ...
              <p className="font-body text-[14px] leading-[1.78] text-ink-secondary">{whatWeDo}</p>
            </div>
            ...
            <div>
              ...
              <p className="font-body text-[14px] leading-[1.78] text-ink-secondary">{whoWeServe}</p>
            </div>
          </motion.div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {donations.map((d, i) => (
              <motion.div key={i} {...fadeUp(i * 0.06)}
                ...
                <p className="font-body text-[13px] leading-[1.65] text-ink-secondary">{d.covers}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="max-w-container mx-auto">
        <motion.div {...fadeUp(0)}
          ...
          <div>
            ...
            <h2 className="font-display font-bold text-cream-surface leading-[1.1] tracking-[-0.01em]
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {METHODOLOGY.map((m, i) => (
            <motion.div key={m.num} {...fadeUp(i * 0.09)}
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-7 h-7 rounded-full bg-accent/15 border border-accent/25
                  ...
                  <span className="font-display text-[11px] font-bold text-accent leading-none">{m.num}</span>
                </div>
                <h3 className="font-display font-bold text-cream-surface text-[18px] leading-snug">
                  {m.headline}
                </h3>
              </div>
              <p className="font-body text-[14px] leading-[1.78] text-cream-surface/55">{m.body}</p>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <motion.div {...fadeUp(0)}
          ...
          <div>
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <Button variant="ghost-dark" href="/partner">Become a Partner</Button>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <p className="font-body text-[15px] leading-[1.8] text-cream-surface/70">
              You have read what we do. You know who we serve. The next step is yours.
            </p>
